      ...
      <Spin spinning={loading}>
        <Descriptions column={1} size="small" colon>
          ...
          <Descriptions.Item label="集群状态">
            <Tag color={clusterInfo?.Status?.Phase === "Running" ? "success" : (clusterInfo?.Status?.Phase === "Creating" ? "processing" : "error")}>
              {clusterInfo?.Status?.Phase || "未知"}
            </Tag>
          </Descriptions.Item>
          <Descriptions.Item label="创建时间">
            {clusterInfo?.metadata?.creationTimestamp
              ? dayjs(clusterInfo.metadata.creationTimestamp).format(
                  "YYYY-MM-DD HH:mm:ss"
                )
              : "-"}
          </Descriptions.Item>
