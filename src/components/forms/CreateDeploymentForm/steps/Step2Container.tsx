            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Form.Item
                  ...
                  labelCol={{ style: { width: 120, textAlign: "right" } }}
                >
                  <Input {...field} placeholder="如 nginx:1.25 或 registry.example.com/app:v1" style={{ width: 400 }} />
                </Form.Item>
              )}
            />
          </Col>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Form.Item
                  ...
                  labelCol={{ style: { width: 120, textAlign: "right" } }}
                >
                  <Input placeholder="用逗号分隔，如: /bin/sh,-c" style={{ width: 400 }} onChange={(e) => {
            ...
            <Controller
              ...
              render={({ field }) => (
                <Form.Item
                  ...
                  labelCol={{ style: { width: 120, textAlign: "right" } }}
                >
                  <Input placeholder="用逗号分隔，如: arg1,arg2,arg3" style={{ width: 400 }} onChange={(e) => {
                    const value = e.target.value;
                    if (value) {
                      field.onChange(value.split(",").map(s => s.trim()).filter(Boolean));
                    } else {
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Form.Item
                  ...
                  labelCol={{ style: { width: 120, textAlign: "right" } }}
                >
                  <InputNumber {...field} placeholder="如 80" style={{ width: 400 }} min={1} max={65535} />
                </Form.Item>
              )}
            />
          </Col>
            ...
            <Row gutter={8} key={field.id} style={{ marginBottom: 8 }}>
              <Col span={24}>
                ...
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} placeholder="变量值（静态）" style={{ width: 180, marginRight: 8 }} />
                  )}
                />
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} placeholder="Secret 名称" style={{ width: 180, marginRight: 8 }} />
                  )}
                />
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Input {...field} placeholder="Secret Key" style={{ width: 180, marginRight: 8 }} />
                  )}
                />
                <Button type="link" danger onClick={() => removeEnv(index)}>删除</Button>
              </Col>
            </Row>
          ))}
          <Button type="dashed" onClick={() => appendEnv({ name: "", value: "" })} style={{ width: 200 }}>
            + 添加环境变量
          </Button>
        </Form.Item>
      </Card>
