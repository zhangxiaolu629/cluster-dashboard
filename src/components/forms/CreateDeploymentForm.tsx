  ...
  const generateYaml = (data: DeploymentFormData): string => {
    const deployment: any = {
      ...
      metadata: {
        ...
        labels: data.labels?.reduce((acc, { key, value }) => {
          if (key) acc[key] = value;
          return acc;
        }, {} as Record<string, string>),
        annotations: data.annotations?.reduce((acc, { key, value }) => {
          if (key) acc[key] = value;
          return acc;
        }, {} as Record<string, string>),
      },
    ...
    if (data.strategyType === "RollingUpdate" && (data.maxSurge || data.maxUnavailable)) {
      ...
      if (data.maxSurge !== undefined) deployment.spec.strategy.rollingUpdate.maxSurge = data.maxSurge;
      if (data.maxUnavailable !== undefined) deployment.spec.strategy.rollingUpdate.maxUnavailable = data.maxUnavailable;
    }
    ...
    <FormProvider {...methods}>
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <Title level={4} style={{ margin: 0 }}>新建 Deployment</Title>
          {hasDraft && (
